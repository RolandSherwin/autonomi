        ...
        match event {
                    ...
                    let action_string: String = match &request {
                        Request::Cmd(cmd) => match cmd {
                            ...
                            ant_protocol::messages::Cmd::PeerConsideredAsBad {
                                ...
                                format!(
                                    "Incoming::Request::Cmd::PeerConsideredAsBad::{bad_behaviour}",
                                )
                            }
                        },
                    ...
                    let action_string = match &response {
                        Response::Cmd(cmd_response) => match cmd_response {
                            CmdResponse::Replicate(result) => {
                                format!(
                                    "Incoming::Response::Cmd::Replicate::{}",
                                    result_to_str(result)
                                )
                            }
                            CmdResponse::FreshReplicate(result) => {
                                format!(
                                    "Incoming::Response::Cmd::FreshReplicate::{}",
                                    result_to_str(result)
                                )
                            }
                        ...
                        Response::Query(query_response) => match query_response {
                            ...
                            ant_protocol::messages::QueryResponse::PutRecord { result, .. } => {
                                match result {
                                    ...
                                    Err(e) => format!(
                                        "Incoming::Response::Query::PutRecord::Err::{}",
                                        error_reason(e)
                                    ),
                                }
                            }
                            ...
                            ant_protocol::messages::QueryResponse::GetStoreQuote {
                                ...
                                Err(e) => format!(
                                    "Incoming::Response::Query::GetStoreQuote::Err::{}",
                                    error_reason(e)
                                ),
                            },
                            ...
                            ant_protocol::messages::QueryResponse::GetReplicatedRecord(result) => {
                                match result {
                                    Ok(_) => "Incoming::Response::Query::GetReplicatedRecord::Ok"
                                        .to_string(),
                                    Err(e) => format!(
                                        "Incoming::Response::Query::GetReplicatedRecord::Err::{}",
                                        error_reason(e)
                                    ),
                                }
                            }
            ...
            request_response::Event::OutboundFailure {
                ...
                if let Some(request_type) = self.pending_request_types.remove(&request_id) {
                    ...
                    let request_type_suffix =
                        request_type.trim_start_matches("Outgoing::Request::");
                    connection_action_logging(
                        Some(&peer),
                        &self.self_peer_id,
                        &connection_id,
                        &format!(
                            "RequestResponse::OutboundFailure::RequestType::{request_type_suffix}"
                        ),
                    );
                }
