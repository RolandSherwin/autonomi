                ...
                if peer == *self.swarm.local_peer_id() {
                    ...
                    let request_type = match &req {
                        Request::Cmd(cmd) => match cmd {
                            ...
                            Cmd::PeerConsideredAsBad { .. } => "Outgoing::Request::Cmd::PeerConsideredAsBad",
                        },
                        Request::Query(query) => match query {
                            ...
                            ant_protocol::messages::Query::GetMerkleCandidateQuote { .. } => {
                                "Outgoing::Request::Query::GetMerkleCandidateQuote"
                            }
                        },
                    };
                        ...
                        if let Response::Query(query_resp) = &resp {
                            let action_string: String = match query_resp {
                                QueryResponse::GetReplicatedRecord(result) => match result {
                                    Ok(_) => "Outgoing::Response::Query::GetReplicatedRecord::Ok".to_string(),
                                    Err(e) => format!("Outgoing::Response::Query::GetReplicatedRecord::Err::{}", error_reason(e)),
                                },
                                ...
                                QueryResponse::GetStoreQuote { quote, .. } => match quote {
                                    Ok(_) => "Outgoing::Response::Query::GetStoreQuote::Ok".to_string(),
                                    Err(e) => format!("Outgoing::Response::Query::GetStoreQuote::Err::{}", error_reason(e)),
                                },
                                QueryResponse::PutRecord { result, .. } => match result {
                                    Ok(_) => "Outgoing::Response::Query::PutRecord::Ok".to_string(),
                                    Err(e) => format!("Outgoing::Response::Query::PutRecord::Err::{}", error_reason(e)),
                                },
                                QueryResponse::GetMerkleCandidateQuote(result) => match result {
                                    Ok(_) => "Outgoing::Response::Query::GetMerkleCandidateQuote::Ok".to_string(),
                                    Err(e) => format!("Outgoing::Response::Query::GetMerkleCandidateQuote::Err::{}", error_reason(e)),
                                },
                            };
