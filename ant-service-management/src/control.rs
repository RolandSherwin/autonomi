...
pub trait ServiceControl: Sync + Send {
    ...
    fn service_definition_has_metrics_port(
        &self,
        service_name: &str,
        user_mode: bool,
    ) -> Result<bool>;
...
impl ServiceControl for ServiceController {
        ...
        for (pid, process) in system.processes() {
            if let Some(path) = process.exe() {
                ...
                if bin_path == path {
                    ...
                    return pid.to_string().parse::<u32>().map_err(|err| {
                        Error::NumberParsingFailed {
                            reason: format!("Failed to parse PID {pid} to u32: {err}"),
                        }
                    });
                }
    ...
    fn service_definition_has_metrics_port(
        ...
        let manager_kind =
            ServiceManagerKind::native().map_err(|err| Error::ServiceManagementFailed {
